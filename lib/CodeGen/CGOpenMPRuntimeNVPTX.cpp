...
static llvm::Value *GetNVPTXBlockID(CodeGenFunction &CGF) {
  ...
  llvm::Function * F;
  if (CGF.getTarget().getTriple().getArch() == llvm::Triple::amdgcn) {
    F = M->getFunction("nvvm.read.ptx.sreg.ctaid.x");
    if (!F) F = llvm::Function::Create(
      llvm::FunctionType::get(CGF.Int32Ty, None, false),
      llvm::GlobalVariable::ExternalLinkage,
      "nvvm.read.ptx.sreg.ctaid.x",M);
  } else
    F = llvm::Intrinsic::getDeclaration(M,
...
static llvm::Value *GetNumWorkers(CodeGenFunction &CGF) {
  CGBuilderTy &Bld = CGF.Builder;
  return Bld.CreateNUWSub(GetNVPTXNumThreads(CGF), GetNVPTXWarpSize(CGF),
                          "num_workers");
}
...
void CGOpenMPRuntimeNVPTX::initializeDataSharing(CodeGenFunction &CGF,
  ...
  Address SlotLVAddr = SlotLV.getAddress();
  llvm::PointerType *SlotPtrTy = getDataSharingSlotTy()->getPointerTo();
  if(SlotLVAddr.getAddressSpace() != SlotPtrTy->getAddressSpace())
    SlotLVAddr = CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(SlotLVAddr,
               SlotPtrTy);
  auto *CastedSlot =
      Bld.CreateBitCast(SlotLVAddr, SlotPtrTy).getPointer();
...
CGOpenMPRuntimeNVPTX::createNVPTXRuntimeFunction(unsigned Function) {
  ...
  case OMPRTL_NVPTX__kmpc_data_sharing_environment_begin: {
    llvm::Type *AThrTy = (CGM.getTriple().getArch()==llvm::Triple::amdgcn) ?
      CGM.Int64Ty->getPointerTo() : CGM.Int32Ty->getPointerTo();
  ...
  case OMPRTL_NVPTX__kmpc_data_sharing_environment_end: {
    ...
    llvm::Type *AThrTy = (CGM.getTriple().getArch()==llvm::Triple::amdgcn) ?
      CGM.Int64Ty->getPointerTo() : CGM.Int32Ty->getPointerTo();
    llvm::Type *TypeParams[] = {SlotTy->getPointerTo()->getPointerTo(),
                       CGM.VoidPtrPtrTy, CGM.VoidPtrPtrTy, AThrTy, CGM.Int32Ty};
    llvm::FunctionType *FnTy =
        llvm::FunctionType::get(CGM.VoidTy, TypeParams, /*isVarArg*/ false);
    RTLFn =
...
CGOpenMPRuntimeNVPTX::outlineTargetDirective(const OMPExecutableDirective &D,
  ...
  // A dedicated function for kernel parameters
  ...
  if (isOpenMPTeamsDirective(TD.getDirectiveKind())) {
    ...
    LValue ArgLVal = WrapperCGF.MakeAddrLValue(
        LocalAddr, ScratchpadArg->getType(), LValueBaseInfo(AlignmentSource::Decl));
  ...
  WrapperCGF.EmitCallOrInvoke(OutlinedFn, CallArgs);
...
public:
  ...
  /// \brief Get the name of the capture helper.
  StringRef getHelperName() const override { return "_omp_simd_outlined_"; }
...
void CGOpenMPRuntimeNVPTX::createDataSharingPerFunctionInfrastructure(
  ...
  QualType AThrQTy = (CGM.getTriple().getArch()==llvm::Triple::amdgcn) ?
      Ctx.getIntTypeForBitwidth(/*DestWidth=*/64, /*Signed=*/false):
      Ctx.getIntTypeForBitwidth(/*DestWidth=*/32, /*Signed=*/false);
  ArgImplDecls.push_back(
      ImplicitParamDecl(Ctx, /*DC=*/nullptr, SourceLocation(),
                        &Ctx.Idents.get("data_share_saved_slot"),
                        Ctx.getPointerType(SlotPtrTy),
  ...
  auto *Fn = llvm::Function::Create(
      CGM.getTypes().GetFunctionType(CGFI), llvm::GlobalValue::InternalLinkage,
      EnclosingCGF.CurFn->getName() + "_data_share", &CGM.getModule());
  ...
  auto &&L0ParallelGen = [this, &DSI, MasterRD, &Ctx, SavedSlot, SavedStack,
    ...
    auto *DataShareAddr = Bld.CreateCall(
  ...
  if (InL0()) {
    ...
    if (CGF.getTarget().getTriple().getArch() == llvm::Triple::amdgcn) {
      ...
      ZeroAddr = CGF.Builder.CreatePointerBitCastOrAddrSpaceCast(
                       ZeroAddr, Fn->getFunctionType()->getParamType(1));
    }
    llvm::SmallVector<llvm::Value *, 16> OutlinedFnArgs;
...
void CGOpenMPRuntimeNVPTX::emitSimdCall(CodeGenFunction &CGF,
  ...
  auto &&SeqGen = [Fn, &CapturedVars](CodeGenFunction &CGF, PrePostActionTy &) {
    ...
    Bld.CreateStore(Bld.getInt32(0), LaneId);
    //CGF.InitTempAlloca(NumLanes, Bld.getInt32(/*C=*/1));
    Bld.CreateStore(Bld.getInt32(1), NumLanes);
...
llvm::Function *CGOpenMPRuntimeNVPTX::emitRegistrationFunction() {
  ...
  if (CGM.getTriple().getArch()==llvm::Triple::amdgcn) {
    AThrTy= CGM.Int64Ty;
    AThrAlign = Ctx.getTypeAlignInChars( 
  ...
  for (auto &DS : DataSharingFunctionInfoMap) {
    ...
    llvm::BasicBlock::iterator FirstInst = HeaderBB.getFirstInsertionPt();
    SmallVector<llvm::Value *, 16> InitArgs;
    unsigned int AS = CGM.getDataLayout().getAllocaAddrSpace();
    llvm::AllocaInst* SlotSaved  = 
        new llvm::AllocaInst(SlotTy->getPointerTo(),AS,/*ArraySize=*/nullptr,
                             PointerAlign, "data_share_slot_saved", &*FirstInst);
    llvm::AllocaInst* StackSaved  = 
        new llvm::AllocaInst(CGM.VoidPtrTy,AS,/*ArraySize=*/nullptr, PointerAlign,
                             "data_share_stack_saved", &*FirstInst);
    llvm::AllocaInst* FrameSaved = 
        new llvm::AllocaInst(CGM.VoidPtrTy,AS, /*ArraySize=*/nullptr, PointerAlign,
                             "data_share_frame_saved", &*FirstInst);
    llvm::AllocaInst* ThreadSaved = 
        new llvm::AllocaInst(AThrTy,AS, /*ArraySize=*/nullptr, AThrAlign,
                             "data_share_active_thd_saved", &*FirstInst);
    // For amdgcn Alloca returns AS5 so we must cat it to generic    
    if (CGM.getTriple().getArch() == llvm::Triple::amdgcn) {
      llvm::CastInst * SlotSavedCasted = 
        llvm::CastInst::Create(llvm::CastInst::AddrSpaceCast, 
        SlotSaved, SlotTy->getPointerTo()->getPointerTo(), 
        "data_share_slot_saved_casted", &*FirstInst);
      llvm::CastInst * StackSavedCasted = 
        llvm::CastInst::Create(llvm::CastInst::AddrSpaceCast, 
        StackSaved, CGM.VoidPtrTy->getPointerTo(), 
        "data_share_stack_saved_casted", &*FirstInst);
      llvm::CastInst * FrameSavedCasted = 
        llvm::CastInst::Create(llvm::CastInst::AddrSpaceCast, 
        FrameSaved, CGM.VoidPtrTy->getPointerTo(), 
        "data_share_frame_saved_casted", &*FirstInst);
      llvm::CastInst * ThreadSavedCasted = 
        llvm::CastInst::Create(llvm::CastInst::AddrSpaceCast, 
        ThreadSaved, AThrTy->getPointerTo(), 
        "data_share_active_thr_saved_casted", &*FirstInst);
      InitArgs.push_back(SlotSavedCasted);
      InitArgs.push_back(StackSavedCasted);
      InitArgs.push_back(FrameSavedCasted);
      InitArgs.push_back(ThreadSavedCasted);
    } else { 
      InitArgs.push_back(SlotSaved);
      InitArgs.push_back(StackSaved);
      InitArgs.push_back(FrameSaved);
      InitArgs.push_back(ThreadSaved);
    }
    ...
    for (auto &VR : DSI.ValuesToBeReplaced) {
      ...
      if (!IsReference) {
        InitArgs.push_back(new llvm::AllocaInst(
            Replacement->getType(),AS, /*ArraySize=*/nullptr, PointerAlign,
    ...
    for (auto &Arg : InitArgs) {
      ...
      if (FArgTy && ArgTy &&
        ...
        Arg = llvm::CastInst::Create(llvm::CastInst::AddrSpaceCast, Arg, FArgTy,
                                     "_data_share_addrspace_cast", InsertPtr);
        ++FArg;
        continue;
      }
